...
Public Class Returning
    ...
    Public Sub RefreshReturningData()
        ...
        Dim preservedTransaction As String = String.Empty
        Try
            If DataGridView1.SelectedRows.Count > 0 Then
                preservedTransaction = Convert.ToString(DataGridView1.SelectedRows(0).Cells("TransactionReceipt").Value)
            End If
        Catch
            preservedTransaction = String.Empty
        End Try
        ...
        Try
            ...
            If Not String.IsNullOrWhiteSpace(preservedTransaction) Then
                Dim found As Boolean = False
                For i As Integer = 0 To DataGridView1.Rows.Count - 1
                    Dim cellVal = If(DataGridView1.Rows(i).Cells("TransactionReceipt").Value, Nothing)
                    If cellVal IsNot Nothing AndAlso cellVal.ToString() = preservedTransaction Then
                        DataGridView1.ClearSelection()
        ...
        If String.IsNullOrWhiteSpace(preservedTransaction) AndAlso DataGridView1.SelectedRows.Count = 0 AndAlso String.IsNullOrWhiteSpace(txttransactionreceipt.Text) Then
            clear_details_only()
        End If
    End Sub
    ...
    Private Sub btnedit_Click(sender As Object, e As EventArgs) Handles btnedit.Click
        ...
        Try
            ...
            Try
                System.Threading.Tasks.Task.Run(Sub()
                                                    'SmsNotifier.ProcessPenaltyNotifications()
                                                End Sub)
            Catch
            End Try
...
newValue:=$"New Status: {bookStatus}, New Accession: {newAccessionStatus}"
        ...
        Catch ex As Exception
            trans?.Rollback()
            MessageBox.Show("Error editing status: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
    ...
    Private Sub DataGridView1_RowPrePaint(sender As Object, e As DataGridViewRowPrePaintEventArgs) Handles DataGridView1.RowPrePaint
        If e.RowIndex >= 0 AndAlso e.RowIndex < DataGridView1.Rows.Count Then
            ...
            Dim borrowerStatus As String = row.Cells("BorrowerStatus").Value?.ToString().Trim().ToUpper()
            Dim bookStatus As String = If(DataGridView1.Columns.Contains("Status"), row.Cells("Status").Value?.ToString().Trim().ToUpper(), String.Empty)


            If borrowerStatus = "PENALIZED" Then
                row.DefaultCellStyle.BackColor = Color.LightGreen

            ElseIf Not String.IsNullOrWhiteSpace(bookStatus) AndAlso (bookStatus.StartsWith("OVERDUE") OrElse bookStatus.Contains("DAMAGED") OrElse bookStatus.StartsWith("LOST")) Then
    ...
    Private Sub DataGridView1_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        ...
        Try
            Dim statusVal As String = selectedRow.Cells("Status").Value?.ToString()
            rboverdue.Checked = False
            rbdamage.Checked = False
            rblost.Checked = False
            cbdamage.SelectedIndex = -1

            If Not String.IsNullOrWhiteSpace(statusVal) Then
                statusVal = statusVal.Trim()
                If statusVal.StartsWith("Overdue", StringComparison.OrdinalIgnoreCase) Then
                    rboverdue.Checked = True
                ElseIf statusVal.StartsWith("Damaged", StringComparison.OrdinalIgnoreCase) OrElse statusVal.IndexOf("Damaged", StringComparison.OrdinalIgnoreCase) >= 0 Then
                    rbdamage.Checked = True

                    Dim openIdx As Integer = statusVal.IndexOf("(")
                    Dim closeIdx As Integer = statusVal.IndexOf(")")
                    If openIdx >= 0 AndAlso closeIdx > openIdx Then
                        Dim level As String = statusVal.Substring(openIdx + 1, closeIdx - openIdx - 1).Trim()
                        For i As Integer = 0 To cbdamage.Items.Count - 1
                            If String.Equals(cbdamage.Items(i).ToString(), level, StringComparison.OrdinalIgnoreCase) Then
                                cbdamage.SelectedIndex = i
                                Exit For
                            End If
                        Next
                    End If
                ElseIf statusVal.StartsWith("Lost", StringComparison.OrdinalIgnoreCase) Then
                    rblost.Checked = True
                End If
            End If
        Catch
    ...
    Private Function GetPenaltyStatus(ByVal transactionNo As String) As String
        ...
        Using con As New MySqlConnection(GlobalVarsModule.connectionString)
            Try
                ...
                Dim updateNotPenalized As String = "UPDATE returning_tbl SET BorrowerStatus = 'NOT PENALIZED' WHERE TransactionReceipt = @transNo"
                Using upd As New MySqlCommand(updateNotPenalized, con)
                    upd.Parameters.AddWithValue("@transNo", transactionNo)
                    upd.ExecuteNonQuery()
                End Using
