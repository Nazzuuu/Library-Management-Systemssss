...
Public Class Penalty
    ...
    Private Sub txtsearch_TextChanged(sender As Object, e As EventArgs) Handles txtsearch.TextChanged
        ...
        If txtsearch.Text.Trim() <> String.Empty Then
            ...
            ' Prefer searching by transaction receipt column
            Dim candidates = New String() {"transactionreceipt", "TransactionReceipt", "transactionno", "TransactionNo", "transaction_id", "transactionid"}
            ...
            If Not String.IsNullOrEmpty(found) Then
                dt.DefaultView.RowFilter = String.Format("CONVERT([{0}], System.String) LIKE '%{1}%'", found, q)
            Else
                ' Fallback: no transaction column, do nothing or clear filter
    ...
    Private Sub btnpenalized_Click(sender As Object, e As EventArgs) Handles btnpenalized.Click
        ...
        If cbstatus Is Nothing OrElse String.IsNullOrWhiteSpace(cbstatus.Text) Then
            MessageBox.Show("Please select a status (Paid, Unpaid or Replaced) before penalizing.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Return
        End If

        Dim selectedStatusUpper As String = cbstatus.Text.Trim().ToUpperInvariant()
        If selectedStatusUpper <> "PAID" AndAlso selectedStatusUpper <> "UNPAID" AndAlso selectedStatusUpper <> "REPLACED" Then
            MessageBox.Show("Please select a valid status: Paid, Unpaid or Replaced.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Return
        End If
        ...
        Dim resultingBorrowerStatus As String = If(selectedStatusUpper = "UNPAID", "NOT PENALIZED", "PENALIZED")

        Using con As New MySqlConnection(connectionString)
            Dim updatePenaltyQuery As String = "UPDATE `penalty_tbl` SET `BorrowerStatus` = @borrowerStatus WHERE `TransactionReceipt` = @transNo"
            Dim updateReturningQuery As String = "UPDATE `returning_tbl` SET `BorrowerStatus` = @borrowerStatus WHERE `TransactionReceipt` = @transNo"

            Try
                con.Open()

                Using cmdPenalty As New MySqlCommand(updatePenaltyQuery, con)
                    cmdPenalty.Parameters.AddWithValue("@borrowerStatus", resultingBorrowerStatus)
                ...
                Using cmdReturning As New MySqlCommand(updateReturningQuery, con)
                    cmdReturning.Parameters.AddWithValue("@borrowerStatus", resultingBorrowerStatus)
        ...
        Try
            UpdatePurStatusForTransaction(transNo, selectedStatusUpper)
        Catch ex As Exception
            MessageBox.Show("PUR Status Update Error: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End Try
        ...
        description:=$"Transaction {transNo} payment status set to {selectedStatusUpper}; BorrowerStatus set to {resultingBorrowerStatus} in penalty_tbl and returning_tbl.",
        recordID:=transNo,
        oldValue:=$"Status: NOT PENALIZED | Fee: {oldFeeStatus}",
        newValue:=$"Payment Status: {selectedStatusUpper} | BorrowerStatus: {resultingBorrowerStatus} | Amount: {enteredFee.ToString("N2")}"
        )
        ...
        MessageBox.Show($"Transaction: {transNo} has been recorded as {selectedStatusUpper}. BorrowerStatus: {resultingBorrowerStatus}.", "Penalty Status Updated", MessageBoxButtons.OK, MessageBoxIcon.Information)

        lblborrowerstatus.Text = resultingBorrowerStatus
    ...
    Private Sub UpdatePurStatusForTransaction(ByVal transactionNo As String, ByVal selectedStatus As String)
        Dim normalizedStatus As String = selectedStatus.Trim().ToUpperInvariant()
        If normalizedStatus <> "PAID" AndAlso normalizedStatus <> "UNPAID" AndAlso normalizedStatus <> "REPLACED" Then
            Return
        End If
        ...
        Using con As New MySqlConnection(GlobalVarsModule.connectionString)
            con.Open()
            Using cmd As New MySqlCommand("UPDATE pur_tbl SET Status=@status WHERE TRIM(TransactionReceipt)=@transNo", con)
        ...
        If updatedRows > 0 Then Return

        Try
            Dim purForm As New PUR()
            purForm.refreshPUR("ALL")
        Catch
        End Try

        Using con2 As New MySqlConnection(GlobalVarsModule.connectionString)
            con2.Open()
            Using cmd2 As New MySqlCommand("UPDATE pur_tbl SET Status=@status WHERE TRIM(TransactionReceipt)=@transNo", con2)
                cmd2.Parameters.AddWithValue("@status", normalizedStatus)
                cmd2.Parameters.AddWithValue("@transNo", transactionNo.Trim())
                updatedRows = cmd2.ExecuteNonQuery()
            End Using
        End Using
    End Sub
    ...
    Private Sub LinkLabel1_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LinkLabel1.LinkClicked
        Try
            ...
            Dim purForm = Application.OpenForms.OfType(Of PUR)().FirstOrDefault()
            ...
            refreshpenalty()
        Catch ex As Exception
            MessageBox.Show("Error opening PUR view: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
