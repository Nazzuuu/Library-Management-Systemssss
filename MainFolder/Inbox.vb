...
Public Class Inbox
    Private Sub Inbox_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim query As String = "SELECT ID, FullName, DueDate, Body, COALESCE(`Date`, CreatedAt) AS `Date` FROM `inbox_tbl` ORDER BY COALESCE(`Date`, CreatedAt) DESC"
                                                          ...
                                                          Try
                                                              ...
                                                              If DataGridView1.Columns.Contains("DueDate") Then
                                                                  ...
                                                                  For Each dgvr As DataGridViewRow In DataGridView1.Rows
                                                                      Try
                                                                          Dim raw = dgvr.Cells("DueDate").Value
                                                                          If raw Is Nothing OrElse IsDBNull(raw) OrElse String.IsNullOrWhiteSpace(raw.ToString()) Then
                                                                              dgvr.Cells("DueDate").Value = ""
                                                                          Else
                                                                              Dim dt As DateTime
                                                                              If DateTime.TryParse(raw.ToString(), dt) Then
                                                                                  dgvr.Cells("DueDate").Value = dt.ToString("yyyy-MM-dd")
                                                                              Else
                                                              ...
                                                              If DataGridView1.Columns.Contains("Body") Then
                                                                  DataGridView1.Columns("Body").Visible = False
                                                                  For Each dgvr As DataGridViewRow In DataGridView1.Rows
                                                                      Try
                                                                          Dim bodyVal = dgvr.Cells("Body").Value
                                                                          If bodyVal IsNot Nothing AndAlso Not IsDBNull(bodyVal) Then
                                                                              Dim txt = bodyVal.ToString()
                                                                              If DataGridView1.Columns.Contains("FullName") Then dgvr.Cells("FullName").ToolTipText = txt
                                                                              If DataGridView1.Columns.Contains("DueDate") Then dgvr.Cells("DueDate").ToolTipText = txt
                                                                              If DataGridView1.Columns.Contains("Date") Then dgvr.Cells("Date").ToolTipText = txt
                                                                          End If
                                                                      Catch
                                                                      End Try
                                                                  Next
